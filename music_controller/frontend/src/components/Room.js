  ...
  getCurrentSong() {
    fetch("/spotify/current-song")
      .then((response) => {
        if (!response.ok) {
          console.error(
            "Failed to fetch the current song, response status:",
            response.status
          );
          throw new Error("Failed to fetch the current song");
        }
      ...
      .then((text) => {
        if (text) {
          ...
          this.setState({ song: {} }); // Reset the song state or handle as needed
        }
      })
      .catch((error) => {
